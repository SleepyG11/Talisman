...
if Talisman.config_file.break_infinity then
	...
	function check_and_set_high_score(score, amt)
      ...
      G:save_settings()
		...
		--going to hold off on modifying this until proper save loading exists
	end
	...
	function inc_career_stat(stat, mod)
		...
		G.PROFILES[G.SETTINGS.profile].career_stats[stat] = G.PROFILES[G.SETTINGS.profile].career_stats[stat]
			+ (mod or 0)
	...
	function scale_number(number, scale, max, e_switch_point)
		if not Big then
			return sn(number, scale, max, e_switch_point)
		end
		scale = to_big(scale)
		G.E_SWITCH_POINT = G.E_SWITCH_POINT or 100000000000
...
function unify_exp(n)
	return string.sub(tostring(n) .. "                     ", 1, 21)
end
function split_number_array(str)
	local result = {}
...
if Big and Big.m then
	function to_big_string(v)
		if not v or not v.m then
			return v
		end
		return "T:b:" .. unify_exp(v.e) .. ":" .. unify_exp(v.m)
	end
	function from_big_string(s)
		if type(s) == "nil" then
			return nil
		end
		return type(s) == "string"
				and setmetatable({ m = tonumber(s:sub(27, 47)), e = tonumber(s:sub(5, 25)) }, BigMeta):normalized()
			or to_big_raw(s)
	end
	function to_big_raw(x, y)
		return Big:new(x, y)
	end
	function to_big(x, y)
		if type(x) == "nil" then
		...
		if type(x) == "string" and x:sub(1, 4) == "T:b:" then
			return x
		else
			return to_big_string(to_big_raw(x, y))
		end
	end
elseif Big and Big.array then
	function to_big_string(v)
		if not v or not v.array then
			return v
		end
		return "T:o:" .. tostring(v.sign) .. ":" .. table.concat(v.array, ",")
	end
	function from_big_string(s)
		return s
				and setmetatable({ array = split_number_array(s:sub(7)), sign = tonumber(s:sub(5, 5)) or 1 }, OmegaMeta):normalize()
			or nil
	end
	function to_big_raw(x, y)
		local result = Big:create(x)
	...
	function to_big(x, y)
		if type(x) == "string" and x:sub(1, 4) == "T:o:" then
			return x
		else
			return to_big_string(to_big_raw(x, y))
		end
...
function resolve_big_string(i)
	return type(i) == "string" and from_big_string(i) or i
end
...
if not Talisman.F_NO_COROUTINE then
			...
			if coroutine.status(G.SCORING_COROUTINE) == "dead" or tal_aborted then
				...
				if not G.OVERLAY_MENU then
									...
									nodes = {
										{
											...
											config = {
												object = DynaText({
													string = { { ref_table = G.scoring_text, ref_value = 1 } },
									...
									nodes = {
										{
											...
											config = {
												object = DynaText({
													string = { { ref_table = G.scoring_text, ref_value = 2 } },
													colours = { G.C.UI.TEXT_LIGHT },
													shadow = true,
													pop_in = 0,
													scale = 0.4,
									...
									nodes = {
										{
											...
											config = {
												object = DynaText({
													string = { { ref_table = G.scoring_text, ref_value = 3 } },
													colours = { G.C.UI.TEXT_LIGHT },
													shadow = true,
													pop_in = 0,
													scale = 0.4,
									...
									nodes = {
										{
											...
											config = {
												object = DynaText({
													string = { { ref_table = G.scoring_text, ref_value = 4 } },
													colours = { G.C.UI.TEXT_LIGHT },
													shadow = true,
													pop_in = 0,
													scale = 0.4,
					...
					if G.OVERLAY_MENU and G.scoring_text then
						...
						G.scoring_text[2] = localize("talisman_string_F")
							.. tostring(totalCalcs)
							.. " ("
							.. tostring(number_format(G.CURRENT_CALC_TIME))
							.. "s)"
						G.scoring_text[3] = localize("talisman_string_G") .. tostring(jokersYetToScore)
						G.scoring_text[4] = localize("talisman_string_H")
							.. tostring(G.GAME.LAST_CALCS or localize("talisman_string_I"))
							.. " ("
							.. tostring(G.GAME.LAST_CALC_TIME and number_format(G.GAME.LAST_CALC_TIME) or "???")
							.. "s)"
					end
				end
	...
	function Card:calculate_joker(context)
		...
		if
			...
			and ((love.timer.getTime() - G.LAST_SCORING_YIELD) > TIME_BETWEEN_SCORING_FRAMES)
...
function Card:start_materialize(a, b, c)
	if
		Talisman.config_file.disable_anims
		and (Talisman.calculating_joker or Talisman.calculating_score or Talisman.calculating_card)
	then
		return
	end
...
function Card:get_chip_x_bonus()
	...
	if (SMODS.multiplicative_stacking(self.ability.x_chips or 1, self.ability.perma_x_chips or 0) or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_e_bonus()
	...
	if (self.ability.e_chips or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_ee_bonus()
	...
	if (self.ability.ee_chips or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_eee_bonus()
	...
	if (self.ability.eee_chips or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_hyper_bonus()
	...
	if type(self.ability.hyper_chips) ~= "table" then
		return { 0, 0 }
	end
	if self.ability.hyper_chips[1] <= 0 or self.ability.hyper_chips[2] <= 0 then
		return { 0, 0 }
	end
...
function Card:get_chip_e_mult()
	...
	if (self.ability.e_mult or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_ee_mult()
	...
	if (self.ability.ee_mult or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_eee_mult()
	...
	if (self.ability.eee_mult or 0) <= 1 then
		return 0
	end
...
function Card:get_chip_hyper_mult()
	...
	if type(self.ability.hyper_mult) ~= "table" then
		return { 0, 0 }
	end
	if self.ability.hyper_mult[1] <= 0 or self.ability.hyper_mult[2] <= 0 then
		return { 0, 0 }
	end
			...
			if not effect.remove_default_message then
				if from_edition then
					card_eval_status_text(
						...
						percent,
						nil,
						{ message = "X" .. amount, colour = G.C.EDITION, edition = true }
					)
			...
			if not effect.remove_default_message then
				if from_edition then
					card_eval_status_text(scored_card, "jokers", nil, percent, nil, {
						message = (amount[1] > 5 and ("{" .. amount[1] .. "}") or string.rep("^", amount[1]))
							.. amount[2],
						colour = G.C.EDITION,
						edition = true,
					})
		...
		if (key == "hyper_mult" or key == "hypermult" or key == "hypermult_mod") and type(amount) == "table" then
			...
			if not effect.remove_default_message then
				if from_edition then
					card_eval_status_text(scored_card, "jokers", nil, percent, nil, {
						message = (
							(amount[1] > 5 and ("{" .. amount[1] .. "}") or string.rep("^", amount[1])) .. amount[2]
						)
							.. " "
							.. localize("k_mult"),
						colour = G.C.EDITION,
...
if Big then
	...
	local function fbsfr(func, source_obj)
		if source_obj then
			return function(t, ...)
				local obj = type(t) == "string" and from_big_string(t) or to_big_raw(t)
				return source_obj[func](obj, ...)
			end
		end
		return function(t, ...)
			local obj = type(t) == "string" and from_big_string(t) or to_big_raw(t)
	...
	local function fbsfp(func, source_obj)
		if source_obj then
			return function(t, ...)
				local obj = type(t) == "string" and from_big_string(t) or to_big_raw(t)
				return to_big_string(source_obj[func](obj, ...))
			end
		end
		return function(t, ...)
			local obj = type(t) == "string" and from_big_string(t) or to_big_raw(t)
			return to_big_string(obj[func](obj, ...))
		end
	end
	...
	for _, func_name in ipairs({
		...
		original_string_meta[func_name] = fbsfr(func_name, OmegaMeta or BigMeta)
	end
	for _, func_name in ipairs({
		"__add",
		"__sub",
		...
		original_string_meta[func_name] = fbsfp(func_name, OmegaMeta or BigMeta)
	end

	original_string_meta.__index = function(t, key)
		return rawget(string_indexable_funcs, key) or rawget(original_string_meta, key) or rawget(string, key)
	end

	debug.setmetatable("", shallow_copy(original_string_meta))
end
